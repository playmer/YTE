...
namespace YTE
{
  ...
  {
    DX12CreatePipelineDataSet(//std::shared_ptr<vkhlf::PipelineCache> aPipelineCache,
    ...
    DX12CreatePipelineDataSet()
    {

    }

    DX12CreatePipelineDataSet(std::string aName, std::string aErrorMessage)
      : mName(aName)
      , mErrorMessage(aErrorMessage)
    {
  ...
  public:
    ...
    Dx12Shader(DX12CreatePipelineDataSet& aInfo, DX12ViewData* aView);
    ~Dx12Shader() override;

    static DX12CreatePipelineDataSet CreateInfo(std::string &aName,
                                              Dx12RenderedSurface *aSurface,
                                              //std::shared_ptr<vkhlf::PipelineLayout> aLayout,
                                              Dx12ShaderDescriptions &aDescriptions,
                                              bool aReload);

    void Load(DX12CreatePipelineDataSet& aInfo);

    void Reload(DX12CreatePipelineDataSet& aInfo);
